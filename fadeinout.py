...
def fade_in_out(ecra, cor, largura, altura, tempo):
    ...
    for alpha in range(0, 255, 5):  # Ajuste o passo (5) para a velocidade de fade desejada
        fade_surface.set_alpha(alpha)
        ecra.blit(fade_surface, (0, 0))
        pygame.display.update()
        pygame.time.delay(int(tempo / 51))  # Ajuste a duração conforme necessário
    ...
    for alpha in range(255, -1, -5):
        ...
        ecra.blit(fade_surface, (0, 0))
        pygame.display.update()
        pygame.time.delay(int(tempo / 51))